...
export default function BastSheet({ data, paperSize = 'f4' }: BastSheetProps) {
  ...
  return (
    <div 
      ...
      className="mx-auto bg-white p-[15mm] text-black font-serif text-[10pt] leading-tight box-border print:p-[13mm] print:shadow-none print:m-0 print:w-full print:min-h-0"
    >
      {/* Header */}
        ...
        <table className="w-full border-collapse border border-slate-400 font-sans text-[9.5pt]">
          <thead>
            <tr className="bg-slate-200 text-slate-800 text-center">
              <th className="border border-slate-400 py-1 px-1.5 w-6" rowSpan={2}>No</th>
              <th className="border border-slate-400 py-1 px-2 text-left" rowSpan={2}>Komponen / Kelengkapan</th>
              <th className="border border-slate-400 py-1 px-1 text-[8pt]" colSpan={3}>Yg Menyerahkan</th>
              <th className="border border-slate-400 py-1 px-1 text-[8pt]" colSpan={3}>Yg Menerima</th>
              <th className="border border-slate-400 py-1 px-2 text-left w-32" rowSpan={2}>Catatan Kondisi</th>
            </tr>
            <tr className="bg-slate-200 text-slate-800 text-center text-[8pt]">
              <th className="border border-slate-400 py-0.5 w-6">B</th>
              <th className="border border-slate-400 py-0.5 w-6">R</th>
              <th className="border border-slate-400 py-0.5 w-6">T</th>
              <th className="border border-slate-400 py-0.5 w-6">B</th>
              <th className="border border-slate-400 py-0.5 w-6">R</th>
              <th className="border border-slate-400 py-0.5 w-6">T</th>
            </tr>
            ...
            {Object.entries(grouped).map(([kategori, list], groupIdx) => (
              <React.Fragment key={kategori}>
                <tr className="bg-slate-100/80 font-bold text-slate-900">
                  <td colSpan={9} className="border border-slate-400 px-2 py-0.5 text-[9.5pt]">
                ...
                {list.map((item, itemIdx) => {
                  ...
                  return (
                    <tr key={item.id} className="hover:bg-slate-50">
                      ...
                      <td className="border border-slate-400 px-2 py-0.5 text-slate-800 leading-tight">
                        {item.nama}
                      </td>
                      <td className="border border-slate-400 text-center py-0.5 font-bold text-emerald-700 text-[9pt]">
                        {isBaik ? '✓' : ''}
                      </td>
                      <td className="border border-slate-400 text-center py-0.5 font-bold text-rose-600 text-[9pt]">
                        {isRusak ? '✗' : ''}
                      </td>
                      <td className="border border-slate-400 text-center py-0.5 font-bold text-slate-500 text-[9pt]">
                        {isTidakAda ? '—' : ''}
                      </td>
                      <td className="border border-slate-400 text-center py-0.5 font-bold text-emerald-700 text-[9pt]">
                        {isBaik2 ? '✓' : ''}
                      ...
                      <td className="border border-slate-400 px-2 py-0.5 text-slate-700 text-[8pt] leading-tight">
                        {val.status === '' ? '' : (val.catatan || (isBaik ? 'Lengkap & Normal' : isRusak ? 'Rusak/Perlu Perbaikan' : 'Tidak Diserahkan'))}
                      </td>
                    </tr>
                  );
                })}
